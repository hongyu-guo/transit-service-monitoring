...
class ChoiceModel(object):
    ...
    def calculate_path_utility(self, beta):
        ...
        self.tb_path_attribute['path_utility'] += beta[1] * self.tb_path_attribute['transfer_over_dist']
        self.tb_path_attribute['path_utility'] += beta[2] * self.tb_path_attribute['no_of_transfer']
...
class Passenger(object):
    ...
    def save_state_tapout(self, pax_data_frame, arrival_time, OD_state):
        ...
        info = pax_data_frame[['pax_origin', 'pax_destination', 'pax_tapin_time', 'pax_number']].rename(
...
class Queue(object):
    ...
    def update_waiting_queue_on_arrival(self, transfer_passengers):
        ...
        if len(self.passenger_queue) > 0:
            self.passenger_queue = pd.concat([self.passenger_queue, transfer_passengers])
        else:
    ...
    def update_waiting_queue_on_departure(self, event_time):
        ...
        if len(self.passenger_queue) > 0:
            self.passenger_queue = pd.concat([self.passenger_queue, new_tap_in])
        else:
    ...
    def save_state(self, station, queue_line, queue_line_direction, queue_time_start, queue_time_end,
        ...
        info = pd.DataFrame({'queue_id': self.queue_id, 'station_id': station,
...
class Carrier(object):
    ...
    def load_passenger(self, station, event_time, passenger_state):
        ...
        if (station, self.carrier_line, self.carrier_direction) in _DefaultValues.DEFAULT_CAP_2_STATION:
            if _DefaultValues.EFFECTIVE_CAP_TRUE:
                if len(_DefaultValues.link_disrupt):
                    wait_pax = \
                    ...
                    wait_pax = _queue.passenger_queue.loc[(_queue.passenger_queue['iti_source_arrival_time'] < event_time) &
                                                          (_queue.passenger_queue['iti_end_station'].apply(int).isin(self.carrier_serve_stations))]
                self.carrier_capacity = int(math.ceil(self.carrier_car_no * self.cap_para[0] + self.cap_para[1] * self.load
                                                      + self.cap_para[2] * len(wait_pax)))
            else:
                self.carrier_capacity = int(math.ceil(self.carrier_car_no * self.cap_para[0])) # new version: always use the DEFAULT_CAR_CAP_1
        else:
            self.carrier_capacity = int(math.ceil(self.carrier_car_no * self.cap_para[0]))
        ...
        if len(_queue.passenger_queue)>0:  # if there is waiting passengers
            ...
            if len(to_be_assigned_pax) > 0:  # if there is waiting to be assigned passengers
                if residual_current > 0:  # if there is capacity to load passengers
                    ...
                    self.onboard_passengers = pd.concat([self.onboard_passengers, boarded_pax])
    ...
    def save_state(self, link_start, link_end, link_entry_time, carrier_state):
        # write attributes to csv file for statistical analysis and visualization
        info = pd.DataFrame({'carrier_id': self.carrier_id, 'carrier_line': self.carrier_line,
        ...
        if self.event_type == 0:  # arrival event (carrier offload pax, pax join queue)
            ...
            if _queue is not None:  # due to the inconsistency between the raw file of network and time table
                ...
                if _carrier.carrier_last_station != self.event_station:
                    ...
                    link_end = self.NETWORK.loc[(self.NETWORK.carrier_line == _carrier.carrier_line) &
                                                (self.NETWORK.carrier_direction == _carrier.carrier_direction) &
                                                (self.NETWORK.link_start == link_start)].link_end.values[0]
...
class NPMModel(object):
    def __init__(self, input_files, beta, path_share, static, cap_para = None, info_print_num = 5000):
        self.info_print_num = info_print_num
        self.OD_state = []
        ...
        if cap_para:
            ...
            self.cap_para = [_DefaultValues.DEFAULT_CAR_CAP_1, _DefaultValues.EFFECTIVE_CAP_PARA_LOAD, _DefaultValues.EFFECTIVE_CAP_PARA_WAIT]
        if static == False:
    ...
    def run_assignment(self,out_put_all):
        # global EVENTS
        tic_o = time.time()
        tic = time.time()
        ...
        return OD_state, passenger_state, station_state, carrier_state
