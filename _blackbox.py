...
def search(f, box, n, m, batch, resfile,resfile_not_sort,
    ...
    def boxtocube(y):
        return [(y[i]-box[i][0])/(box[i][1]-box[i][0]) for i in range(d)]
    def cubetobox(x):
        return [box[i][0]+(box[i][1]-box[i][0])*x[i] for i in range(d)]
    # generating latin hypercube
    points = np.zeros((n, d+1))
    x_list = ['x' + str(i + 1) for i in range(d)]
    col = x_list + ['target']
    if len(CONTINUE) >= n:
        points[0:n, :] = CONTINUE.loc[:,col].values[0:n]
        points[:, 0:-1] = list(map(boxtocube, points[:, 0:-1]))
    else:
    ...
    if len(CONTINUE) < n:
        ...
        temp_pd = pd.DataFrame(points_save, columns=col)
        temp_pd.to_csv(resfile_not_sort, index=False)
    ...
    if len(CONTINUE) >= n:
        #initialize points
        points = CONTINUE.loc[:,col].values[0:len(CONTINUE)]
        points[:, 0:-1] = list(map(boxtocube, points[:, 0:-1]))
        points[:, -1] = points[:, -1] / fmax
        i = len(CONTINUE) - n
    else:
    ...
    while i< num_iter:
        ...
        print(f'total num local search: {num_iter}, current num: {i}')
        if d > 1:
