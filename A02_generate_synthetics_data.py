...
def Generate_synthetic_demand(opt_model, beta_all, output_file_name):
    ...
    OD_state.to_csv(opt_model.input_file_path + 'exit_demand_synthesized_' + output_file_name + '.csv', index=False)
...
if __name__ == "__main__":
    ...
    # 'reference' cap_high_sensitive  'beta_random' # beta_random  beta_deterministic beta_deterministic_upper cap_no_sensitive cap_high_sensitive

    # if output_file_name == 'beta_deterministic':
    #     beta_bound_1 = [[bd[0] * 10, bd[1] * 10] for bd in beta_bound[:4]]
    #     beta_bound = beta_bound_1 + beta_bound[4:]
    #     intial_beta_list = [0.5 * bound[0] + 0.5 * bound[1] for bound in beta_bound]
    #
    ...
    True_beta_list = [-0.147, -1.271, -0.573, -3.679]

    beta_all = True_beta_list
    save_para = pd.DataFrame({'in_vehicle_time': [True_beta_list[0]], 'transfer_over_dist': [True_beta_list[1]],
                              'no_of_transfer': [True_beta_list[2]], 'commonality_factor': [True_beta_list[3]]})
    save_para.to_csv('data/true_para_' + output_file_name + '.csv', index=False)
