...
class OptimizationModel(object):
    ...
    def BlackboxFunc(self, beta):
        ...
        obj = w1 * exit_flow_diff + w2 * entropy
        print('current beta is:', beta, 'current obj', obj)
        ...
        if len(OD_state) > 0:
            ...
            for index, row in flow_merge.iterrows():
                if len(row['tt_bin_x']) == len(row['tt_bin_y']):
                    flow_merge.loc[index, 'KL_div'] = stats.entropy(row['tt_bin_x'], row['tt_bin_y'])
...
def CORS_RBF_solver(opt_model, bound, random_seed, output_name, num_LT_search, num_follow_search, num_of_parallel,
    ...
    if CONTINUE:
        if os.path.exists(output_name):
            out_put = pd.read_csv(output_name).dropna()
        else:
            temp_dict = {}
            for x_id in range(len(bound)):
                x_name = 'x' + str(x_id + 1)
                temp_dict[x_name] = []
            temp_dict['target'] = []
            out_put = pd.DataFrame(temp_dict)
    else:
        temp_dict = {}
        for x_id in range(len(bound)):
            x_name = 'x' + str(x_id + 1)
    ...
    _blackbox.search(f=opt_model.BlackboxFunc,  # given function
                     ...
                     random_seed=random_seed, CONTINUE=out_put)  # text file where results will be saved
...
if __name__ == "__main__":
    ...
    num_of_parallel = 1
    num_of_iter = 102
    beta_bound = [[-2, 0], [-5, 0], [-3, 0], [-10, 0]]  # include cap para
    initial_beta_list = [0.5 * bound[0] + 0.5 * bound[1] for bound in beta_bound]
    ...
    my_opt.Load_input_files(output_file_name, GENERATE=False)


    CONTINUE = True
    random_seed = 1
    output_name = 'output/' + 'CORS_RBF_results_' + str(random_seed) + '_' + output_file_name + '.csv'
    print('CORS_RBF_solver', output_name)
    fraction_global = 0.2
    num_LT_search = math.ceil(fraction_global * num_of_iter)
    print('number of global data points', num_LT_search)
    num_follow_search = math.floor((1 - fraction_global) * num_of_iter)
    CORS_RBF_solver(my_opt, beta_bound, random_seed, output_name, num_LT_search, num_follow_search, num_of_parallel,
                    CONTINUE)  # define the parameters in function: paulknysh_bb_solver
